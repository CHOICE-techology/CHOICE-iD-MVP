...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // Simulate secure backend verification
    setTimeout(() => {
      setIsLoading(false);
      onLogin();
    }, 1500);
    ...
    <div className="min-h-screen bg-light flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-slate-200/50 p-8 border border-slate-100 animate-fade-in">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-dark mb-2">Social Activation</h1>
          <p className="text-slate-500">Verify your identity to unlock CHOICE iD features.</p>
        </div>

        <div className="space-y-4">
          <button 
            onClick={handleGoogle}
            className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 text-dark font-semibold py-3 px-4 rounded-xl transition-all duration-200"
          >
