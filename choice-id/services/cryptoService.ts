...
export const mockVerifyPhysicalDocument = async () => {
    await new Promise(resolve => setTimeout(resolve, 3000)); // Simulate OCR/Verification time
    return {
        verified: true,
        issuer: 'Choice AI Verifier',
        confidence: 0.98
    };
};
...
export const mockAnalyzeWalletHistory = async (address: string) => {
  ...
  date.setMonth(date.getMonth() - (seed % 12));

  // Determine expertise based on 'random' seed
  const expertises = [];
...
export const mockConnectSocial = async (platform: string, handle: string) => {
  ...
  const missions = [
    ...
    "Creating digital art that transcends boundaries",
    "Connecting global communities on-chain"
  ];
...
export const calculateJobMatch = (job: Job, score: number, credentials: VerifiableCredential[]): { score: number, reason: string } => {
    ...
    // 3. Wallet History / Expertise (20% weight)
    const walletVC = credentials.find(vc => vc.type.includes('WalletHistoryCredential'));
    if (walletVC) {
        match += 20;
        const years = new Date().getFullYear() - new Date(walletVC.credentialSubject.firstTxDate).getFullYear();
        if (years > 2) reasons.push(`Verified ${years} years on-chain activity`);
    }

    return { 
        score: Math.floor(match), 
        reason: reasons.length > 0 ? reasons.join(". ") : "Partial match based on profile." 
    };
};
...
export const mockGenerateCV = async (identity: UserIdentity): Promise<GeneratedCV> => {
    ...
    const skills = [
        ...(walletVC?.credentialSubject.expertises || []),
        ...eduVCs.map(vc => vc.credentialSubject.courseName),
    ...
    if (walletVC) {
        experience.push({
            ...
            duration: `${new Date(walletVC.credentialSubject.firstTxDate).getFullYear()} - Present`
        });
    }
    socialVCs.forEach(vc => {
        experience.push({
            role: `Content Creator / Influencer`,
            company: vc.credentialSubject.platform,
    ...
    const education = [
        ...realWorldVCs.map(vc => ({
            degree: vc.credentialSubject.documentType,
            institution: vc.credentialSubject.issuer || "External Authority",
            year: "Verified"
        })),
        ...eduVCs.map(vc => ({
            degree: vc.credentialSubject.level + " Certificate",
...
export const mockGenerateBio = async (identity: UserIdentity): Promise<string> => {
    const docs = identity.credentials.filter(vc => vc.type.includes('PhysicalCredential')).map(vc => vc.credentialSubject.documentType);
    const socials = identity.credentials.filter(vc => vc.type.includes('SocialCredential')).map(vc => vc.credentialSubject.platform);
